...
**一款用户主动启动的 Android 电池性能测试与曲线记录工具**
...
Battery Tester 是一款面向 Android 设备的电池性能辅助测试 App。它可以在用户主动启动后记录放电或充电期间的电量、电流、电压、温度等数据，并生成可视化曲线与 CSV 数据文件。
...
Battery Tester 是辅助测试工具，不是后台省电工具，也不是自动压测工具。
...
如果你本地暂时只安装了 Android 35，也可以把项目中的 `compileSdk` 和 `targetSdk` 调整为 35，并安装：
...
- 估算功率不是外部功率计测量值。
- 当前 MVP 使用 CPU 负载和屏幕负载，暂未实现 GPU 渲染负载。
